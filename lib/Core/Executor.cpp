...
cl::opt<std::string> TargetFunction("target-function",
...
void Executor::executeCall(ExecutionState &state, KInstruction *ki, Function *f,
  ...
  if (f && f->isDeclaration()) {
    ...
    if (kf->function->getName() == TargetFunction) {
      ...
      for (auto &filename : res) {
        ...
        Parser *P = Parser::Create(filename, MBResult.get(), Builder, false);
        P->SetMaxErrors(20);

        ref<Expr> summary = ConstantExpr::alloc(1, 1);
        while (Decl *D = P->ParseTopLevelDecl()) {

          if (ArrayDecl *ad = dyn_cast<ArrayDecl>(D)) {
            auto width = ad->Range * ad->Root->size;

            auto name = ad->Root->getName();
            ...
            if (funcName != TargetFunction) {
              continue;
            }

            auto argno = [&]() {
              auto i = name.find_last_of("_") + 1;
              auto len = name.size() - i;
              return stoi(name.substr(i, len));
            }();

            auto expr = Expr::createTempRead(ad->Root, width);

            auto eqExpr = EqExpr::alloc(expr, arguments[argno]);
            fakeState.addConstraint(eqExpr);

          } else if (QueryCommand *qc = dyn_cast<QueryCommand>(D)) {
            for (auto c : qc->Constraints) {

              // fakeState.addConstraint(c);
              summary = AndExpr::alloc(summary, c);
            }
          }
